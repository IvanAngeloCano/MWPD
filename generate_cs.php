...
try {
    ...
    $createTableSQL = "CREATE TABLE IF NOT EXISTS `bm_cs_files` (
    ...
    $checkColumnSQL = "SHOW COLUMNS FROM bm_cs_files LIKE 'filename'";
    $stmt = $pdo->query($checkColumnSQL);
    if ($stmt->rowCount() == 0) {
        // Add the filename column
        $pdo->exec("ALTER TABLE bm_cs_files ADD COLUMN filename VARCHAR(255) AFTER file_name");
        // Update existing records to copy file_name to filename
        $pdo->exec("UPDATE bm_cs_files SET filename = file_name WHERE filename IS NULL");
    }
    
    $checkColumnSQL = "SHOW COLUMNS FROM bm_cs_files LIKE 'filepath'";
    $stmt = $pdo->query($checkColumnSQL);
    if ($stmt->rowCount() == 0) {
        // Add the filepath column
        $pdo->exec("ALTER TABLE bm_cs_files ADD COLUMN filepath VARCHAR(255) AFTER file_path");
        // Update existing records to copy file_path to filepath
        $pdo->exec("UPDATE bm_cs_files SET filepath = file_path WHERE filepath IS NULL");
    }
    ...
    if ($existingFile) {
        ...
        $filename = isset($existingFile['filename']) ? $existingFile['filename'] : 
                   (isset($existingFile['file_name']) ? $existingFile['file_name'] : $data['last_name'] . "_CRITICAL_SKILLS.docx");
        ...
        $stmtUpdate = $pdo->prepare("UPDATE bm_cs_files SET file_path = ?, filepath = ? WHERE bmid = ?");
        ...
        $stmtInsert = $pdo->prepare("INSERT INTO bm_cs_files (bmid, file_name, file_path, filename, filepath) VALUES (?, ?, ?, ?, ?)");
    ...
    if ($return_var === 0) {
        ...
        register_shutdown_function(function() use ($savePath, $pdfPath) {
            ...
            if (file_exists($pdfPath)) {
                unlink($pdfPath); // Delete PDF file
            }
        });
