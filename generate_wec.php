...
try {
    ...
    $createTableSQL = "CREATE TABLE IF NOT EXISTS `bm_wec_files` (
    ...
    $checkColumnSQL = "SHOW COLUMNS FROM bm_wec_files LIKE 'filename'";
    $stmt = $pdo->query($checkColumnSQL);
    if ($stmt->rowCount() == 0) {
        // Add the filename column
        $pdo->exec("ALTER TABLE bm_wec_files ADD COLUMN filename VARCHAR(255) AFTER file_name");
        // Update existing records to copy file_name to filename
        $pdo->exec("UPDATE bm_wec_files SET filename = file_name WHERE filename IS NULL");
    }
    
    $checkColumnSQL = "SHOW COLUMNS FROM bm_wec_files LIKE 'filepath'";
    $stmt = $pdo->query($checkColumnSQL);
    if ($stmt->rowCount() == 0) {
        // Add the filepath column
        $pdo->exec("ALTER TABLE bm_wec_files ADD COLUMN filepath VARCHAR(255) AFTER file_path");
        // Update existing records to copy file_path to filepath
        $pdo->exec("UPDATE bm_wec_files SET filepath = file_path WHERE filepath IS NULL");
    }
    ...
    $template = new TemplateProcessor('WATCHLISTED_EMPLOYER_CLEARANCE_TEMPLATE.docx');
    ...
    $stmtCheck = $pdo->prepare("SELECT * FROM bm_wec_files WHERE bmid = ?");
    ...
    if ($existingFile) {
        ...
        $filename = isset($existingFile['filename']) ? $existingFile['filename'] : 
                   (isset($existingFile['file_name']) ? $existingFile['file_name'] : $data['last_name'] . "_WATCHLISTED_EMPLOYER.docx");
        ...
        $stmtUpdate = $pdo->prepare("UPDATE bm_wec_files SET file_path = ?, filepath = ? WHERE bmid = ?");
        $stmtUpdate->execute([$savePath, $savePath, $bmid]);

    } else {
        // If no file exists, create a new one in the generated_files folder
        $filename = $data['last_name'] . "_WATCHLISTED_EMPLOYER_CLEARANCE" . ".docx";
        $savePath = $generatedFilesDir . '/' . $filename;

        $template->saveAs($savePath);

        $stmtInsert = $pdo->prepare("INSERT INTO bm_wec_files (bmid, file_name, file_path, filename, filepath) VALUES (?, ?, ?, ?, ?)");
