...
try {
    ...
    $tableExists = false;
    $tables = $pdo->query("SHOW TABLES LIKE 'bm_sp_files'")->fetchAll();
    if (count($tables) > 0) {
        $tableExists = true;
    }
    ...
    if (!$tableExists) {
        $createTableSQL = "CREATE TABLE IF NOT EXISTS `bm_sp_files` (
        ...
        $checkColumn = $pdo->query("SHOW COLUMNS FROM bm_sp_files LIKE 'filename'")->fetchAll();
        ...
        $checkColumn = $pdo->query("SHOW COLUMNS FROM bm_sp_files LIKE 'filepath'")->fetchAll();
        if (count($checkColumn) > 0) {
            $filepathExists = true;
        }
        ...
        if (!$filenameExists) {
            $pdo->exec("ALTER TABLE bm_sp_files ADD COLUMN filename VARCHAR(255)");
        }
        
        if (!$filepathExists) {
            $pdo->exec("ALTER TABLE bm_sp_files ADD COLUMN filepath VARCHAR(255)");
        }
    }
    ...
    if ($existingFile) {
        ...
        $filename = isset($existingFile['filename']) ? $existingFile['filename'] : 
                   (isset($existingFile['file_name']) ? $existingFile['file_name'] : $data['last_name'] . "_SEAFARERS_POSITION.docx");
